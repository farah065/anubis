...
namespace MoreMountains.Tools
{
	public class MMSMPlaylistManager : MMMonoBehaviour
	{
		...
		#region CONTROLS
			...
			/// <summary>
			/// Plays the song at the specified index, and at a specific time, with an option to fade in.
			/// </summary>
			/// <param name="index"></param>
			/// <param name="time"></param>
			/// <param name="withFade"></param>
			/// <returns></returns>
			public virtual void PlaySongAt(int songIndex, float time, bool withFade = false)
			{
				PlaySongAt(songIndex);
				_currentlyPlayingAudioSource.time = time;
				if (withFade)
				{
					MMSoundManager.Instance.FadeSound(_currentlyPlayingAudioSource, FadeDuration,
						0f, _currentlyPlayingAudioSource.volume, FadeTween, false);
